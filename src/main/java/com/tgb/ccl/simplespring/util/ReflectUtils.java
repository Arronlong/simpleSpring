...
public class ReflectUtils {
	...
	public static boolean existField(Field field,Class<?> clazz){
		...
		try {
			...
			logger.warn("判断某个字段是否在{}类中时出错：{}",clazz,e);
		}
				...
				for (Field field : fields) {
					...
					if(value != null){
						...
						try {
							...
							logger.warn("map转Bean时出错，字段名：{}，类{}：{}",fieldName,clazz.getSimpleName(),e);
						}
					}
				}
			}
			...
			if(obj != null){
				for (Field field : fields) {
					...
					try{
						...
						logger.warn("Bean转map时出错，字段名：{}，类{}：{}",field.getName(),clazz.getSimpleName(),e);
					}
				}
			}
